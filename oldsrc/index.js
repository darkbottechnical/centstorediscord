...
class CentStoreDiscord {
    ...
    handleMessage(msg, rinfo) {
        ...
        if (parsed.access && parsed.access == "public") {
            ...
            this.channels.public.get(channelNo).pushMessage(decoded);
            this.window.webContents.send("channels.update.one");
        } else if (parsed.access && parsed.access == "private") {
            if (!parsed.data || !parsed.data.from || !parsed.data.from.mac)
                return;
            const remoteMac = parsed.data.from.mac;
            const messageData = parsed.data;
            this.channels.private.get(remoteMac).pushMessage(messageData);
        }
    }
